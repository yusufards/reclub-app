...
@section('content')
    ...
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="bg-white p-4 md:p-6 rounded-3xl shadow-lg mb-6 border border-gray-100">
            ...
            <p class="text-sm text-gray-500 mb-4">Klik di mana saja pada peta untuk mencari lapangan di area spesifik.</p>
            ...
            <form id="searchForm" method="GET" action="{{ route('venues.search') }}"
                ...
                <div class="md:col-span-2 flex items-end">
                    <button type="button" onclick="getLocationAndSubmit()"
                        class="w-full bg-emerald-600 text-white py-2.5 rounded-xl hover:bg-emerald-700 font-bold shadow-md transition text-sm flex justify-center items-center gap-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="flex flex-col lg:grid lg:grid-cols-3 gap-6 lg:h-[600px]">
            ...
            {{-- MAP CONTAINER --}}
            <div
                class="h-[350px] lg:h-full lg:col-span-2 bg-gray-100 rounded-3xl shadow-md overflow-hidden relative border border-gray-200 order-1">
                <div id="map" class="w-full h-full z-10 cursor-crosshair"></div>
                <div id="loadingMap"
                    class="absolute inset-0 bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center z-20 hidden">
            ...
            {{-- LIST CONTAINER --}}
            <div
                class="h-[450px] lg:h-full bg-white rounded-3xl shadow-md overflow-hidden flex flex-col border border-gray-200 order-2">
                    ...
                    @forelse($venues as $venue)
                        <div class="border border-gray-100 rounded-2xl p-4 hover:shadow-md hover:border-emerald-200 transition cursor-pointer bg-white group"
                            ...
                            <div class="flex justify-between items-start">
                                <div class="w-full">
                                    <h4
                                        class="font-bold text-gray-900 group-hover:text-emerald-600 transition text-sm md:text-base">
                                        {{ $venue->name }}</h4>
                                    <p class="text-xs text-gray-500 mb-2 truncate">{{ $venue->city }}</p>
                                </div>
                                @if(isset($venue->distance))
                                    <div
                                        class="shrink-0 ml-2 bg-emerald-50 text-emerald-700 text-[10px] px-2 py-1 rounded-lg font-bold border border-emerald-100 whitespace-nowrap">
                            ...
                            <button type="button" onclick="selectVenue(event, {{ $venue->id }})"
    ...
    <script>
        ...
        // Check if user has searched with coordinates
        const searchedLat = "{{ request('lat') }}";
        const searchedLng = "{{ request('lng') }}";
        ...
        // --- 2. MAP CLICK EVENT (FITUR BARU) ---
        map.on('click', function (e) {
            const lat = e.latlng.lat;
            const lng = e.latlng.lng;

            // 1. Hapus marker manual sebelumnya jika ada
            if (manualMarker) {
                map.removeLayer(manualMarker);
            }

            // 2. Tambah marker baru di lokasi klik
            manualMarker = L.marker([lat, lng], { icon: clickIcon }).addTo(map);

            // 3. Isi Hidden Input Form
            document.getElementById('lat').value = lat;
            document.getElementById('lng').value = lng;

            // 4. Tampilkan Popup dengan Koordinat & Tombol Cari
            const popupContent = `
                <div class="text-center font-sans p-1">
                    <p class="text-xs font-bold text-gray-500 mb-1">Lokasi Dipilih</p>
                    <p class="text-[10px] text-gray-400 mb-2">${lat.toFixed(5)}, ${lng.toFixed(5)}</p>
                    <button onclick="submitSearch()" class="bg-gray-800 text-white text-xs font-bold py-1.5 px-3 rounded hover:bg-black transition w-full">
        ...
        // --- 5. INITIAL ZOOM/CENTER LOGIC ---
        if (searchedLat && searchedLng) {
            // Jika hasil pencarian berdasarkan lokasi (GPS/Klik), pasang marker merah & center di sana
            manualMarker = L.marker([searchedLat, searchedLng], { icon: clickIcon })
                .addTo(map)
                .bindPopup(`<div class="text-center text-xs font-bold">Lokasi Pencarian</div>`);

            map.setView([searchedLat, searchedLng], 14);

            // Buka popup marker manual sebentar agar user sadar
            setTimeout(() => manualMarker.openPopup(), 500);
        }
        else if (hasVenues) {
            // Jika hanya browsing kota/default, fit bounds ke semua venue
